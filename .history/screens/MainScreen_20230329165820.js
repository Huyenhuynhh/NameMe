...
const MainScreen = ({ navigation }) => {
  ...
  const handleClassPress = (className) => {
    // Navigate to the corresponding quiz
    navigation.navigate(`${className} Quiz`);
  };
    ...
    {loading ? (
      ...
      classes.map(({ className, description }) => (
        <ClassBox
          key={className}
          title={className}
          onPress={() => handleClassPress(className)}
        >
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <View style={styles.classBox}>
          <TouchableOpacity onPress={() => navigation.navigate("CMSC312")}>
            <View style={styles.classBoxContent}>
